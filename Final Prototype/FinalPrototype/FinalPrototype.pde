...
void draw() {
  ...
  textSize(50);
  fill(186, 0, 255);
  text("Double click ANYWHERE to submit", width/2, height-inchToPix(.55f));
  textSize(30);
  text("except for the square and control panel", width/2, height-inchToPix(.2f));
}
...
{  
  ...
  {
    if(mouseX > prevPosX && (screenRotation < 90))
    {
    ...
    else if (mouseX < prevPosX && (screenRotation > -90))
    {
...
public void mouseClicked(MouseEvent evt) {
  if (evt.getCount() == 2 && !(overCircle()) && !(overSquare()) ){
